...
export function CMDProveedores() {
  ...
  const handleDelete = async (index) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await axios.post(`/api/ProductEngineering/eliminarProveedor?id=${index}`);
        if (response.status === 200) {
          await Swal.fire('Eliminado', 'El proveedor ha sido eliminado correctamente', 'success');
          getProveedores();
        } else {
  ...
  const filteredProviders = proveedores
    .map(extractData)
    .filter(provider => 
  ...
  const currentUsers = filteredProviders.slice(indexOfFirstEvento, indexOfLastEvento);
  ...
  const handleEditProvider = (providerId) => {
    const providerToEdit = proveedores.find(provider => provider.id === providerId); // Buscar el usuario en el estado
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex items-center mb-4 text-sm text-muted-foreground">
        ...
        <a href="/configuraciones/cmd/proveedores" className="font-bold hover:underline text-primary">Administrador de proveedores</a>
      </div>
      ...
      <div className="flex justify-between mb-4">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={handleCleanForm}><PackageOpen className="h-4 w-4" /> Añadir proveedor</Button>
          </DialogTrigger>
          ...
          <DialogContent onInteractOutside={(event) => event.preventDefault()} className="border-none p-0 overflow-y-auto no-scrollbar" style={{
            ...
            <DialogHeader>
              <DialogTitle className="flex justify-center items-center text-center">Nuevo proveedor</DialogTitle>
              <DialogDescription className="flex justify-center items-center text-center">Ingresa los detalles del proveedor.</DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              <Button type="submit" disabled={!nombre} >Agregar proveedor</Button>
            </DialogFooter>
            </form>
          </DialogContent>
        </Dialog>
        ...
        <TableBody>
          {currentUsers.length >0 ?( currentUsers.map((user,index) => (
            <TableRow key={index}>
              ...
              <TableCell className="text-right">
                ...
                <Dialog open={openEdit} onOpenChange={setOpenEdit}>
                    <DialogTrigger asChild>
                      <Button onClick={() => handleEditProvider(user.id)} variant="outline" size="sm">Editar</Button>
                    </DialogTrigger>
                    <DialogContent onInteractOutside={(event) => event.preventDefault()} className="border-none p-0 overflow-y-auto no-scrollbar" style={{
                        width: "100%", // Ajusta el ancho
                        maxWidth: "500px", // Límite del ancho
            ...
            <DialogHeader>
              ...
              <DialogDescription>Actualiza los detalles necesarios del proveedor.</DialogDescription>
            </DialogHeader>
